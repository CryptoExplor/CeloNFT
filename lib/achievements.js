...
export class AchievementsManager {
   ...
   * Uses ownerOf + tokenTraits so it works even if ERC721Enumerable is not implemented.
   * This mirrors the robust (but slower) logic from main.old.js.
   */
  async fetchUserNFTs(userAddress) {
    try {
      // First get how many NFTs the user owns
      ...
      // Then scan totalSupply and collect tokens owned by this user
      ...
      const ownedNFTs = results.filter((nft) => nft !== null);
      console.log('AchievementsManager.fetchUserNFTs found NFTs:', ownedNFTs.length);
      return ownedNFTs;
    } catch (error) {
  ...
  async loadAchievementsBottom(userAddress, contractDetails, userMintCount, userNFTs = []) {
    ...
    // Always ensure we have up-to-date NFT data for accurate achievement checking
    ...
    // Derive mint count from NFTs if explicit count is missing/zero
    const effectiveMintCount = (userMintCount && userMintCount > 0)
      ? userMintCount
      : (loadedNFTs ? loadedNFTs.length : 0);
    console.log('Achievements effectiveMintCount:', effectiveMintCount);
    
    let unlockedCount = 0;
    
    const html = this.achievements.map(achievement => {
      // Use mint-count-based or NFT-based checks depending on achievement type
      let unlocked = false;
      if (['first_mint', 'five_mints', 'ten_mints', 'top_collector'].includes(achievement.id)) {
        unlocked = achievement.check(effectiveMintCount);
      } else {
