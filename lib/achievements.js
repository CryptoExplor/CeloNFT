...
export class AchievementsManager {
   ...
   */
  async getUserAchievements(userAddress, userNFTs, predictionStats) {
    try {
      ...
      const earned = this.achievements
        .filter((achievement) => {
          if (achievement.id === 'top_collector') {
            // For top_collector, we need to pass userMintCount which should be the length of nfts
            return achievement.check(nfts.length);
          }
  ...
  async loadAchievementsBottom(userAddress, contractDetails, userMintCount, userNFTs = []) {
    ...
    if (loadedNFTs.length === 0 && userAddress && contractDetails) {
      try {
        const balance = await readContract(this.wagmiConfig, {
        ...
        if (nftCount > 0) {
          ...
          const results = await Promise.all(promises);
          loadedNFTs = results.filter(nft => nft !== null);
        }
      } catch (e) {
