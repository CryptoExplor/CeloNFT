...
async function initializeApp() {
  try {
    ...
    if (externalBanner && externalBannerText) {
      console.log('📍 Setting up banner...');
      
      if (isFarcasterEnvironment) {
        // In Farcaster → "Open in Browser"
        ...
        console.log('✅ Banner configured: "Open in Browser"');
      } else {
        // Not in Farcaster → "Open in Farcaster"
        externalBanner.href = MINIAPP_URL;
        externalBannerText.textContent = 'Open in Farcaster';
        externalBanner.classList.remove('hidden');
        externalBanner.classList.add('show', 'pulse');
        console.log('✅ Banner configured: "Open in Farcaster" (with pulse)');
      }
      
      // Verify banner is visible
      const styles = window.getComputedStyle(externalBanner);
      console.log('📊 Banner display style:', styles.display);
