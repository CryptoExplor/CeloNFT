 ...
 */
export function saveMintToHistory(tokenId, txHash, userAddress, safeLocalStorage) {
  try {
 ...
 */
function animateCounterHelper(element, start, end, duration) {
...
/**
 * Update supply counters with animation
 */
export async function updateSupplyCounters(
  mintingManager,
  totalMintedStat,
  remainingStat,
  mintBtn,
  ...
  try {
    ...
    if (totalMintedStat) {
      ...
      if (current !== total) {
        animateCounterHelper(totalMintedStat, current, total, 800);
      }
    }
    ...
    if (mintingManager.maxSupply > 0 && total >= mintingManager.maxSupply) {
      ...
      mintBtn.title = "Maximum supply reached";
    }
