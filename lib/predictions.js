...
export class PredictionManager {
   ...
   */
  async verifyPrediction(userAddress, timestamp, getCurrentPrice) {
    try {
      const priceData = await getCurrentPrice();
      const newPrice = priceData.price;

      console.log('Verifying prediction:', { userAddress, timestamp, newPrice });

      // Try API verification first
      try {
        const result = await apiClient.verifyPrediction(userAddress, timestamp, newPrice);
        if (result.success) {
          return result;
        }
      } catch (apiError) {
      ...
      return this.clientSideVerification(timestamp, newPrice);
    } catch (error) {
